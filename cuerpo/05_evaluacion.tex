...
Segunda, el conjunto se regenera automáticamente con cada actualización del corpus, lo que hace la evaluación reproducible y sostenible en el tiempo (requisito \ref{req:cuatro}).
...
Tres \glspl{llm} distintos locales generan, cada uno, una propuesta candidata, y un panel formado por esos mismos modelos (esquema \emph{LLM como juez}) las puntúa y selecciona la mejor. El proceso es reproducible (semilla fija) y se aplica sobre un conjunto de referencia de 500 trabajos reales (250 \gls{tfg} y 250 \gls{tfm}).
...
La calidad del posicionamiento se resume con el rango recíproco medio (MRR) que, al haber un solo relevante, coincide con la precisión media (MAP) y con la ganancia acumulada descontada normalizada:
...
$\mathrm{Hit@1}$ mide la precisión estricta («¿acierta a la primera?»); $\mathrm{Hit@}5$ y $\mathrm{Hit@}10$ reflejan el uso real del sistema, en el que el estudiante inspecciona la primera pantalla de resultados y decide entre varios candidatos; la MRR resume en un único número la calidad del posicionamiento, premiando los primeros puestos; y la nDCG aporta una lectura graduada de esa misma calidad.
...
La estrategia de control (léxica directa) lanza la búsqueda dispersa usando las palabras clave originales del propio trabajo supervisado, sin intervención del modelo de lenguaje: actúa como una \emph{extracción} que aísla la contribución del \gls{llm} en la interpretación de la consulta y como línea base clásica de coincidencia léxica.
...
\begin{table}[htbp]
    ...
    \begin{tabularx}{\textwidth}{l X c c}
        ...
        Léxica directa (control) & Palabras clave originales del trabajo, sin transformación & No & Disperso \\
