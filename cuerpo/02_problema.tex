...
\begin{figure}[htbp]
  ...
  \caption[Perfil público de un PDI en el Portal Científico]{Perfil público de un investigador en el Portal Científico de la \gls{upm}, tal como lo consulta hoy un estudiante: rico y bien presentado para la lectura humana ficha a ficha, pero no pensado para la exploración ni la comparación a escala.}
  \label{fig:portal}
\end{figure}
...
Esta barrera motiva el primer requisito de diseño del sistema: la \emph{adquisición automatizada, incremental y reproducible} de los datos del portal (\ref{req:adquisicion}), que se aborda mediante el objetivo~\ref{obj:adquisicion}.
...
\begin{figure}[htbp]
  ...
  \begin{tikzpicture}
    ...
    \end{axis}
  \end{tikzpicture}
  \caption[Distribución del número de publicaciones por PDI en el perímetro]{Distribución del número de publicaciones por investigador en el perímetro (995 \glspl{pdi}). Los extremos conviven: 219 perfiles sin publicaciones registradas y 147 con más de cien (hasta un máximo de 726), lo que ilustra la heterogeneidad que la representación debe tolerar.}
  \label{fig:densidad}
\end{figure}

A la disparidad de densidad se suma el ruido en el contenido de cada campo.
Los temas de investigación se expresan en vocabularios no controlados, con duplicados, variantes morfológicas y mezcla de idiomas; los resúmenes de las publicaciones presentan formatos idiosincrásicos (cuando no contienen, directamente, metadatos de la fuente en su lugar); y conviven publicaciones duplicadas e incluso fragmentos bibliográficos capturados por error como publicaciones propias.
La Tabla~\ref{tab:ruido} recoge ejemplos reales (anonimizados) de estos fenómenos, extraídos del propio perímetro: todos ellos deben tolerarse o corregirse durante una limpieza cuidadosa antes de construir las representaciones semánticas posteriores, para no contaminarlas.

\begin{table}[htbp]
    \centering
    \caption[Ejemplos reales de heterogeneidad y ruido]{Ejemplos reales (anonimizados) de heterogeneidad y ruido observados en el perímetro. Los nombres propios se han sustituido por marcadores; el resto del dato se reproduce literalmente.}
    \label{tab:ruido}
    \footnotesize
    \begin{tabularx}{\textwidth}{l X}
        \toprule
        \rowcolor{upmblue!8} \colhead{Fenómeno} & \colhead{Ejemplo real (anonimizado)} \\
        \midrule
        Densidad muy dispar & El Investigador~A reúne 726 publicaciones (la más antigua, de 1999), mientras que el Investigador~B acumula solo 15 repartidas a lo largo de 31 años (1994--2025). \\
        Temas duplicados y variantes & Entre los temas de un mismo investigador conviven \texttt{cord-1} y \texttt{cord-19}, o el solapamiento de \texttt{knowledge graph}, \texttt{ontology} y \texttt{linked data}. \\
        Vocabulario no controlado y multilingüe & Una misma publicación se etiqueta a la vez como \texttt{computer science (all)}, \texttt{computer science (miscellaneous)}, \texttt{general computer science} y \texttt{computer science, theory \& methods}, junto al portugués \texttt{ciência da computação} y \texttt{administração pública e de empresas}. \\
        Publicaciones duplicadas & El registro «Repository of indexed ROs» aparece dos veces: una fechada en 2015 y otra sin año válido. \\
        Descripciones que no son resúmenes & El campo de descripción contiene a menudo metadatos de la fuente en lugar de un resumen: \texttt{Deliverable}, \texttt{Proceedings of ICTERI} o \texttt{International Semantic Web Conference (Posters \& Demos)}. \\
        Citas capturadas como publicaciones & Fragmentos bibliográficos colados como publicaciones propias, p.\,ej. «[Apellido], [Inicial]., y [Apellido], [Inicial]. (2017a)», y fechados con un año inválido (0). \\
        \bottomrule
    \end{tabularx}
\end{table}
...
Esta barrera motiva el segundo requisito de diseño: \emph{perfiles normalizados} que agreguen toda la evidencia disponible y toleren datos incompletos (\ref{req:normalizacion}), que se aborda mediante los objetivos~\ref{obj:adquisicion} y~\ref{obj:perfilado}.

\section{La brecha de vocabulario entre estudiante y PDI}
...
Este fenómeno, identificado desde los orígenes de la recuperación de información como el \emph{problema del vocabulario} \citep{furnas1987vocabulary} (la constatación empírica de que dos personas eligen el mismo término para referirse a un mismo objeto con una probabilidad inferior al 20\%), es aquí especialmente severo por su carácter multilingüe, lo que anticipa una limitación importante de las aproximaciones basadas únicamente en coincidencia léxica.
...
Esta barrera motiva el tercer requisito de diseño: un \emph{emparejamiento robusto al desajuste léxico y translingüe} entre consulta y perfil (\ref{req:emparejamiento}), que se aborda mediante los objetivos~\ref{obj:perfilado} y~\ref{obj:emparejamiento}.
...
Esta carencia de verdad de referencia constituye una limitación metodológica relevante para este tipo de herramientas: sin evaluación objetiva se dificulta su validación, su comparación con alternativas y su mejora sistemática y el desarrollo tiende a estancarse en la anécdota.
...
Esta barrera motiva el cuarto requisito de diseño: una \emph{evaluación objetiva y reproducible sin etiquetado manual} (\ref{req:evaluacion}), que se aborda mediante el objetivo~\ref{obj:evaluacion}.
...
Esta barrera motiva el quinto y último requisito de diseño: el \emph{despliegue íntegro en infraestructura propia}, con modelos abiertos y coste marginal nulo por consulta (\ref{req:soberania}), que se aborda mediante el objetivo~\ref{obj:prototipo}.
...
\begin{table}[htbp]
    \centering
    \caption[Correspondencia entre barreras, requisitos y objetivos del sistema]{Correspondencia entre las barreras identificadas, los requisitos de diseño del sistema y los objetivos que los abordan.}
