...
Aun una vez estructurados, los datos distan de ser homogéneos.
...
Esta heterogeneidad tiene una consecuencia metodológica importante\pendiente{simplificar esta oración}: cualquier representación de los investigadores debe ser robusta frente a perfiles incompletos y debe agregar la evidencia disponible (publicaciones, proyectos, temas declarados) en lugar de depender de un único campo bien cumplimentado.
...
Este fenómeno, conocido en recuperación de información como \emph{desajuste de vocabulario} \pendiente{Añadir referencia?}, es aquí especialmente severo por su carácter multilingüe, lo que constituye la razón técnica por la que una solución ingenua basada en búsqueda de palabras clave resulta insuficiente.\duda{Podemos ya afirmar esto?}
Abordarlo exige representaciones que capturen proximidad semántica más allá de la superficie léxica, o bien mecanismos que traduzcan y expandan la consulta del estudiante hacia el registro de los perfiles, idealmente, ambas cosas (se abordan en el Capítulo~\ref{ch:sota}).
...
\begin{table}[htbp]
    \centering
    \caption{Correspondencia entre las barreras identificadas y los requisitos de diseño del sistema.}
    \label{tab:requisitos}
    \begin{tabularx}{\textwidth}{l X X}
        \toprule
        \rowcolor{upmblue!8} \colhead{Id.} & \colhead{Barrera} & \colhead{Requisito derivado} \\
        \midrule
        \reqid{req:uno}    & Fuente pública no operativa (\ref{sec:problema-datos}) & Adquisición automatizada, incremental y reproducible de los datos del portal.\\
        \reqid{req:dos}    & Heterogeneidad y ruido (\ref{sec:problema-heterogeneidad}) & Perfiles normalizados que agreguen toda la evidencia disponible y toleren datos incompletos.\\
        \reqid{req:tres}   & Brecha de vocabulario e idioma (\ref{sec:problema-vocabulario}) & Emparejamiento robusto al desajuste léxico y translingüe entre consulta y perfil.\\
        \reqid{req:cuatro} & Ausencia de verdad de referencia (\ref{sec:problema-groundtruth}) & Evaluación objetiva y reproducible sin etiquetado manual.\\
        \reqid{req:cinco}  & Soberanía del dato y coste (\ref{sec:problema-institucional}) & Despliegue íntegro en infraestructura propia, con modelos abiertos y coste marginal nulo por consulta.\\
        \bottomrule
    \end{tabularx}
\end{table}
\pendiente{Tengo que intentar que se quede en la página anterior, que parece que cabe}