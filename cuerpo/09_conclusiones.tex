...
\begin{itemize}
    ...
    El corpus resultante reúne 172\,621 nodos y 268\,886 relaciones a partir de la actividad pública censada, de los 995 investigadores del perímetro de las cuatro escuelas seleccionadas, 709 quedan indexados por sus temas declarados (índice léxico) y 754 por su biografía y dominios generados (índice semántico).
    ...
    La comparación muestra que las tres estrategias semánticas forman un grupo estadísticamente indistinguible y claramente superior a las léxicas: la híbrida (fusión \gls{rrf} de la señal dispersa y la densa) obtiene los mejores valores puntuales (\gls{mrr} 0,153), seguida de la semántica enriquecida (0,146) y de la directa (0,137), si bien estas diferencias internas no son significativas; frente a ellas, la mejor estrategia léxica se queda en 0,093, una brecha esta sí significativa que confirma el dominio de la recuperación semántica.
...
\begin{mydescription}
    ...
    Dos candidatas claras: el \emph{re-ranking} de los primeros resultados con un modelo más potente (\emph{cross-encoder} o \gls{llm} como juez) y el ajuste fino del modelo de embedding al dominio académico bilingüe utilizando los propios pares $supervisi\acute{o}n-proyecto$ como señal de entrenamiento.
    Una tercera, sugerida por el desajuste temporal observado en la evaluación (Sección~\ref{sec:eval-validez}), es indexar varios resúmenes anuales por investigador (los últimos $N$) junto a la biografía vigente, deduplicando o consolidando los resultados en la recuperación, para recoger también líneas recientes que la fusión global pudiera diluir.
    ...
    \item[Filtros y controles en la interfaz.]
    El prototipo devuelve hoy a los investigadores más afines con independencia de su adscripción. Aunque el sentido del sistema sea precisamente derribar las barreras de ubicación y departamento (más aún con la generalización del trabajo en remoto), un estudiante puede preferir restringir la búsqueda a su propia escuela, a un departamento concreto u otros criterios.
    Ofrecer filtros opcionales (escuela, departamento, idioma de la actividad\ldots) sobre la lista de resultados es una mejora de usabilidad inmediata que no altera el motor de recuperación.
