...
El Portal Científico de la \gls{upm} recoge, para cada miembro del \gls{pdi}, sus publicaciones, proyectos, líneas de investigación y actividad de supervisión.
El problema no es de disponibilidad, sino de \emph{accesibilidad efectiva}: esa información está pensada para ser consultada ficha a ficha, no para ser explorada, comparada o consultada en lenguaje natural.
...
Además, la misma opacidad que dificulta el \emph{encuentro estudiante-tutor} dificulta también la detección de sinergias \emph{entre} investigadores: dos grupos que trabajan en problemas adyacentes pueden no llegar a saberlo nunca.\pendiente{Decir por qué}
...
Este \gls{tfm} se desarrolla en el seno del \gls{maadm} de la \gls{upm} y constituye el núcleo técnico del \gls{pie} \textbf{IE26.6102, «UPM Match: Sistema de Recomendación de Tutores y Colaboraciones»} (convocatoria 2025-2026) \citep{pie_upmmatch_2026}, coordinado desde la \gls{etsisi} por Carlos Badenes Olmedo, con la co-coordinación de Andrea Cimmino Arriaga y un equipo interdisciplinar que incluye profesorado de otras escuelas de la universidad.
El proyecto se adscribe a la línea de innovación \emph{E1: Integración de la IA en las actividades docentes}, con la que la UPM promueve la incorporación de \gls{ia} a sus procesos formativos.

Los objetivos institucionales del proyecto delimitan con precisión el problema que este trabajo aborda: facilitar la elección de tutores afines a partir del análisis de datos objetivos del Portal Científico; aumentar la visibilidad de la información académica del profesorado de forma estructurada; contribuir a equilibrar la distribución de tutorías atendiendo a la afinidad temática; fomentar colaboraciones interdepartamentales mediante el análisis de las redes de coautoría; y ofrecer al estudiante una experiencia personalizada y transparente.
De los resultados comprometidos por el proyecto, este \gls{tfm} materializa el prototipo funcional del sistema de recomendación, la base de datos de perfiles del \gls{pdi} y la evaluación retrospectiva del sistema (el «retroanálisis» previsto en su plan de evaluación, que el Capítulo~\ref{ch:evaluacion} formaliza); la \textcolor{red}{prueba piloto con estudiantes} y las acciones de difusión corresponden al ámbito del proyecto y se recogen aquí como trabajo futuro.\duda{Mejor este párrado en un listado?}

El perímetro inicial del sistema abarca el PDI de cuatro escuelas del ámbito de las Tecnologías de la Información y las Comunicaciones de la UPM: la \acrfull{etsisi}, la \gls{etsii}, la \gls{etsist} y la \gls{etsit}, lo que supone un total de \pendiente{indicar el número final de investigadores, publicaciones y proyectos del corpus}.
...
El objetivo general de este trabajo es \textbf{diseñar, construir y evaluar un sistema de recomendación de tutores académicos a partir de los datos públicos de actividad investigadora de la UPM}, capaz de atender consultas en lenguaje natural y de justificar sus recomendaciones.

Este objetivo general se descompone en los siguientes objetivos específicos:

\begin{itemize}
    \item \textbf{O1. Adquisición y estructuración.} Construir un proceso automatizado, repetible e incremental que adquiera los datos públicos del Portal Científico y los estructure en un modelo de datos explotable.
    \item \textbf{O2. Perfilado semántico.} Generar, para cada investigador, un perfil enriquecido que sintetice su trayectoria: temas de investigación normalizados, dominios de especialización y una biografía investigadora redactada automáticamente.
    \item \textbf{O3. Emparejamiento.} Implementar y contrastar varias estrategias de recuperación (léxica dispersa, semántica densa e híbrida) que emparejen la idea libre de un estudiante con los perfiles de los investigadores, de forma robusta frente a diferencias de idioma y vocabulario.
    \item \textbf{O4. Evaluación objetiva.} Definir un marco de evaluación reproducible que no requiera etiquetado manual, utilizando la historia real de supervisión como verdad de referencia, y emplearlo para comparar las estrategias anteriores.
    \item \textbf{O5. Prototipo.} Materializar el sistema en un prototipo web usable, transparente en sus explicaciones y desplegable íntegramente en infraestructura propia de la institución.
\end{itemize}
...
Las contribuciones principales de este trabajo son cuatro:

\begin{enumerate}
    \item Un \textbf{corpus estructurado y enriquecido} de la actividad investigadora de toda la \gls{upm}, construido íntegramente a partir de fuentes públicas y reconstruible de forma automática.
    \item Una \textbf{metodología de evaluación sin etiquetado manual} para sistemas de recomendación de tutores, basada en proyectos realmente supervisados como consultas y en su supervisor real como respuesta correcta.
    ...
    \duda{No sé si está bien mencionar ya todas, igual eso es para más adelante. Faltaría el tema de Knowledge Graphs y ontologías}
