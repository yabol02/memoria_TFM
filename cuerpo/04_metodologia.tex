...
\begin{mydescription}
    ...
    Esto permite re-ejecutar cualquier etapa de forma aislada e incremental (solo lo nuevo o lo inválido) sin repetir el proceso completo (\ref{req:uno}).
...
La primera etapa convierte el portal público en una colección local y versionada, comprende dos pasos.
El primero es el \emph{censo}: descubrir los investigadores recorriendo las unidades organizativas del portal (escuelas y departamentos), dado que no existe un listado descargable.\duda{Esto es mentira, es justo lo que descargo, pongo otra cosa mejor?}
...
\section{Estructuración: el perfil como grafo}
\label{sec:met-grafo}
...
\textcolor{red}{Además, deja preparado el terreno para la línea futura de detección de colaboraciones, que es esencialmente un problema de análisis de grafos (esto es lo que quiero hacer ahora, igual lo quito).}

Durante esta etapa se aplica una limpieza \emph{conservadora} del texto: solo se transforma aquello que responde a patrones inequívocos de la fuente (por ejemplo, resúmenes con secciones etiquetadas concatenadas), prefiriéndose conservar una imperfección a introducir una corrupción silenciosa (requisito \ref{req:dos}).
\pendiente{Creo que lo cambié ya esto, tengo que repasar el código}

\section{Consolidación y enriquecimiento del perfil}
\label{sec:met-enriquecimiento}

La tercera etapa produce, para cada investigador, un \emph{documento de perfil} autocontenido que agrega su evidencia: posiciones ocupadas, temas normalizados (unificación de variantes y duplicados), publicaciones con sus resúmenes, proyectos recientes, trabajos supervisados en el último periodo y asignaturas impartidas.
Este documento es la unidad de trabajo de todo lo que sigue.
\pendiente{Un PDI es Personal Docente e Investigador. En general, tenemos que hacer más hincapié en estas dos vertientes (en el código también!!!)}

Sobre él se aplica el enriquecimiento lingüístico, que genera dos artefactos complementarios mediante un modelo de lenguaje operado localmente:

\begin{itemize}
    \item Una \textbf{biografía investigadora}: una síntesis en prosa de la trayectoria completa.
    Se construye de forma \emph{incremental y acumulativa}: la producción científica se agrupa cronológicamente y el modelo va refinando un resumen acumulado periodo a periodo.
    Este procedimiento evita exceder la ventana de contexto del modelo con trayectorias largas, equilibra el peso de las distintas etapas de la carrera y produce descripciones homogéneas entre perfiles densos y dispersos.
    El estado intermedio de cada periodo se conserva, lo que aporta trazabilidad sobre cómo se fue construyendo la biografía final.
    \item Una lista acumulativa de \textbf{dominios de especialización}: áreas de conocimiento concisas extraídas de la misma evidencia, con una regla de estabilidad (los dominios ya establecidos se conservan y solo se añaden los genuinamente nuevos) que evita oscilaciones entre ejecuciones.
\end{itemize}
...
El enriquecimiento queda persistido junto al resto del perfil, de modo que las etapas posteriores no dependen de volver a invocar al modelo.

\section{Representación e indexación semántica}
\label{sec:met-indexacion}

La cuarta etapa proyecta cada perfil a un espacio de representación buscable.
Atendiendo al análisis del Capítulo~\ref{ch:sota}, se mantienen deliberadamente \emph{dos}\duda{CUIDADO, esto es ahora y no estoy seguro del todo que lo haya dejado así, que cambié cosas} representaciones complementarias por investigador:
...
\begin{itemize}
    ...
    \item Una \textbf{representación semántica densa} de su biografía investigadora, que captura proximidad conceptual más allá de la superficie léxica y de la lengua en que se exprese la consulta.
\end{itemize}
...
La fase en línea ofrece cuatro estrategias de emparejamiento\duda{CUIDADO, esto lo voy a cambiar}, que se corresponden con las familias revisadas en el estado del arte (Tabla~\ref{tab:posicionamiento}) y que el marco de evaluación comparará en igualdad de condiciones:
